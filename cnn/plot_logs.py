...
def generate_plots(log_file='./train_logs/log_data.json', output_dir='./plots/', output_file='plots.png'):
    ...
    # Extract parameters
    parameters = {
        "episode": [entry["episode"] for entry in log_data],
        "reward": [entry["reward"] for entry in log_data],
        "coverage": [entry["coverage"] for entry in log_data],
        "fairness": [entry["fairness"] for entry in log_data],
        "energy_efficiency": [entry["energy_efficiency"] for entry in log_data]
    }
